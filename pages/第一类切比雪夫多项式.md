...
- ### [[首一切比雪夫多项式]]
	...
	- ### 定理
	  ...
	  $$
		- 定理表明在所有首项系数为1 的 n 次多项式集合 $\widetilde{H}$ 中
		  $$\parallel\widetilde{T}_n\parallel_\infty=\min\parallel P(x)\parallel_\infty$$,
		  所以$\widetilde{T}_n(x)$是$\widetilde{H}_n$ 中最大值最小的多项式，即$P\in\widetilde{H}_n$
		- $$
		  \max_{-1\leqslant x\leqslant1}\mid\widetilde{T}_n(x)\mid=\min_{P\in\widetilde{H}_n^{-1\leqslant x\leqslant1}}\mid P(x)\mid=\frac{1}{2^{n-1}}.
		  $$

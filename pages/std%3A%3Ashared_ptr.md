...
- 所有 成员函数（包括 复制构造函数 和 复制赋值函数 ）可以在不需要额外[[同步]]的情况下由多个线程调用，即使这些实例是副本并共享相同对象的拥有权。如果多个执行线程在没有同步的情况下访问相同的 `shared_ptr` 实例，而这些访问中的任何一个使用了 `shared_ptr` 的非常数成员函数，那么会发生数据竞争；可以使用 `shared_ptr` 的原子函数重载来防止数据竞争。
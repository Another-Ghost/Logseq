...
- ## VM 作为[[缓存]]工具
	...
	- ### VM 的[[局部性]]
		...
		-
- ## VM 作为内存管理工具
	- [[操作系统]]为^^每个[[进程]]提供了一个独立的[[页表]]^^，因而也就是一个独立的[[虚拟地址空间]]。
...
- ## 总结
	- 虚拟内存提供三个重要的功能：
	  logseq.order-list-type:: number
		- 它在主存中自动缓存最近使用的存放磁盘上的虚拟地址空间的内容。虚拟内存缓存中的块叫做页。对磁盘上页的引用会触发缺页，缺页将控制转移到操作系统中的一个．缺页处理程序。缺页处理程序将页面从磁盘复制到主存缓存，如果必要，将写回被驱逐的页。
		  logseq.order-list-type:: number
		- 虚拟内存简化了内存管理，进而又简化了链接、在进程间共享数据、进程的内存分配以及程序加载。
		  logseq.order-list-type:: number
		- 虚拟内存通过在每条页表条目中加入保护位，从而了简化了内存保护。
		  logseq.order-list-type:: number
	- logseq.order-list-type:: number